...
export default function Snowflake(props: any) {
  ...
  function mergeRoster(newHexes: hexagon[]): void {
    hexRoster = hexRoster.concat(newHexes)
  }
  ...
  function growChildren(parent: BranchObject): void {
    for (let i = 1; i < parent.roster.length; i++) {
      ...
      if (childDirection === 2) {
        childLength = randomBounded(1, Math.floor(i / 2))
      } else {
        childLength = randomBounded(1, i)
      }
      const nextBranch = new BranchObject(
        {
          seed: { q: childSeed.q, r: childSeed.r },
  ...
  const mainBranch = new BranchObject(
    { seed: { q: 0, r: 0 }, direction: 0, length: branchLength },
    null,
    'bg-ice'
  )
