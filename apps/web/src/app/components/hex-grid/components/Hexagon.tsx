...
export interface HexProps {
  ...
  hexText?: string
  clickMessage: any
  render?: ([cX, cY]: [number, number]) => React.ReactNode
}
...
export default function Hex(props: HexProps) {
  ...
  const pixiPath = cornerAngles.flatMap((angle) => {
    const theta = calcTheta(angle)
    const x = Math.floor(center.x + hexRadius * Math.cos(theta))
    const y = Math.floor(center.y + hexRadius * Math.sin(theta))
    return [x, y]
  ...
  const draw = useCallback(
    (g: PixiGraphics) => {
      g.beginFill(0xff0000).drawPolygon(pixiPath).endFill()
    },
    [pixiPath]
  )
  ...
  if (props.render) {
    return (
      <Container>
        <Graphics draw={draw} />
        {props.render([center.x, center.y])}
      </Container>
    )
  } else {
